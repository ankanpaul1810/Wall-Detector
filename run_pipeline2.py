...
# We use a small, precise blocklist.
# We ONLY want to block things that are definitely NOT walls.
BLOCK_LIST_CLASSES = [
    "ceiling", "floor", "chair", "table", "person", "sofa", "bed", "shelf"
]
# The allow list is good.
ALLOW_LIST_CLASSES = ["wall", "column", "panel", "building"]

BLOCK_LIST_IDS = []
ALLOW_LIST_IDS = []

MIN_MASK_AREA_RATIO = 0.02 # Mask must be at least 2% of the image
...
def run_pipeline(args):
    ...
    print("Starting AI Wall Mask generation pipeline...")

    # --- 1. File & Device Setup ---
    # Use raise SystemExit to signal a real error back to app1.py
    if not os.path.exists(args.room):
        raise SystemExit(f"Error: Room image not found: {args.room}")
        
    if not os.path.exists(SAM_MODEL_PATH):
        raise SystemExit(f"Error: SAM model not found: {SAM_MODEL_PATH}\nPlease download 'sam_vit_b_01ec64.pth' and place it in the same directory.")
    ...
    try:
        ...
        raise SystemExit(f"Error loading SegFormer: {e}. Check internet and 'transformers' library.")

    # Convert our class names to the model's internal IDs
    ...
    try:
        ...
        raise SystemExit(f"Error loading SAM: {e}. Check 'segment_anything' library or model path.")
    ...
    try:
        ...
        raise SystemExit(f"Error reading room image {args.room}: {e}")
        
    room_np = np.array(room_pil)
    room_h, room_w = room_np.shape[:2]
    room_total_pixels = room_w * room_h
    MIN_MASK_AREA = room_total_pixels * MIN_MASK_AREA_RATIO # Set dynamic min area
    ...
    if room_w > MAX_IMAGE_WIDTH:
        ...
        room_h, room_w = new_h, new_w # Update dimensions
    ...
    try:
        ...
        # Upscale logits to match resized image
        ...
        raise SystemExit(f"Error during SegFormer inference: {e}. (May be Out of Memory)")
    ...
    try:
        ...
        raise SystemExit(f"Error during SAM inference: {e}. (May be Out of Memory)")


    # --- 5. Logic to Select the Correct Wall Mask ---
    ...
    for i, mask_data in enumerate(all_masks):
        ...
        if is_allowed and not is_blocked:
            ...
            # else:
            #     print(f"Skipping mask {i} (Area: {area}, Top Labels: {top_label_names}) -> Not in allow-list.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(1) # Ensure a non-zero exit code on our custom errors
    except Exception as e:
        print(f"An unexpected error occurred: {e}", file=sys.stderr)
        sys.exit(1) # Ensure a non-zero exit code on any other error